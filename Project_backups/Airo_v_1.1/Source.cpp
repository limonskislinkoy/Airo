...
void Load_Materials_to_array(float *arr,int len_array) {
	int start_pos = 0;
	std::fstream matfile;
	std::string matline;
	std::string word;
	matfile.open("test1.mtl", std::ios::in);

	while (std::getline(matfile, matline))
	{
		if (matline[0] == 'K' and matline[1] == 'd') {
			std::stringstream ss(matline);
			
			ss >> word;
			ss >> word;
			arr[start_pos * 3 + 0] = std::stof(word);
			ss >> word;
			arr[start_pos * 3 + 1] = std::stof(word);
			ss >> word;
			arr[start_pos * 3 + 2] = std::stof(word);
...
int main()
{
	...
	float start_color_r = 0.3f;
	float start_color_g = 0.3f;
	float start_color_b = 0.3f;

	std::string line;
	

	const int len_arr_materials=10*3;
	float arr_materials[len_arr_materials];
	Load_Materials_to_array(&arr_materials[0], len_arr_materials);
	std::cout << ": " << arr_materials[0] << " " << arr_materials[1] << " " << arr_materials[2];
	...
	while (std::getline(objfile, line))
	{
		...
		if (line[0] == 'u' and line[1] == 's' and line[2] == 'e' and line[3] == 'm') {
			int number_of_material = std::stoi(line.erase(0, line.find('.')+1))-1;
			//std::cout << number_of_material;
			start_color_r = arr_materials[number_of_material * 3 + 0];
			start_color_g = arr_materials[number_of_material * 3 + 1];
			start_color_b = arr_materials[number_of_material * 3 + 2];
		...
		if (line[0] == 'v' && line[1] == ' ') {
			...
			obj_vertices[start_pos_for_readobj] = start_color_r;
			start_pos_for_readobj++;

			obj_vertices[start_pos_for_readobj] = start_color_g;
			start_pos_for_readobj++;

			obj_vertices[start_pos_for_readobj] = start_color_b;
			start_pos_for_readobj++;
		...
		if (line[0] == 'f') {
			...
			while (gc < 3) {
				...
				//vertices[start_pos_for_writeobj + 3] = obj_vertices[(token - 1) * 6 + 3];
				//vertices[start_pos_for_writeobj + 4] = obj_vertices[(token - 1) * 6 + 4]; // OLD VERSION
				//vertices[start_pos_for_writeobj + 5] = obj_vertices[(token - 1) * 6 + 5];


				vertices[start_pos_for_writeobj + 3] = start_color_r;
				vertices[start_pos_for_writeobj + 4] = start_color_g;
				vertices[start_pos_for_writeobj + 5] = start_color_b;
